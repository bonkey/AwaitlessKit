...
/// Filters out the AwaitlessPromise and AwaitablePromise attributes from the attributes list
func filterAttributes(_ attributes: AttributeListSyntax) -> AttributeListSyntax {
    attributes.filter { attr in
        if case let .attribute(actualAttr) = attr,
           ...
           attrName.name.text == "AwaitlessPromise" || attrName.name.text == "AwaitablePromise"
        {
            return false
        }
        return true
