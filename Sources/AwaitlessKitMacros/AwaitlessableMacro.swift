...
public struct AwaitlessableMacro: MemberMacro, ExtensionMacro {
    ...
    private static func createSyncFunctionWithDefaultImplementation(
        ...
        let parameterCall = if parameters.isEmpty {
            "()"
        } else {
        ...
        let finalBodyContent = if hasReturnValue {
            "return \(bodyContent)"
        } else {
            bodyContent
        }
    ...
    private static func parseExtensionGenerationArgument(from node: AttributeSyntax) -> AwaitlessableExtensionGeneration {
